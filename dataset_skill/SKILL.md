...
description: "Use for RAGFlow dataset tasks: create, list, inspect, update, or delete datasets; upload, list, update, or delete documents; start or stop parsing; check parse status; retrieve chunks with `search.py`; list configured models; manage chat assistants; send chat messages; manage sessions; test connectivity."
user-invocable: true
metadata:
  openclaw:
    requires:
